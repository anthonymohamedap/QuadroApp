...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Lijsten]') AND [c].[name] = N'Beschrijving');
    IF @var3 IS NOT NULL EXEC(N'ALTER TABLE [Lijsten] DROP CONSTRAINT [' + @var3 + '];');
    ALTER TABLE [Lijsten] DROP COLUMN [Beschrijving];
END;
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Leveranciers]') AND [c].[name] = N'Code3');
    IF @var22 IS NOT NULL EXEC(N'ALTER TABLE [Leveranciers] DROP CONSTRAINT [' + @var22 + '];');
    ALTER TABLE [Leveranciers] ALTER COLUMN [Code3] nvarchar(3) NOT NULL;
END;
...
BEGIN
    ALTER TABLE [TypeLijsten] ADD [Beschrijving1] nvarchar(max) NOT NULL DEFAULT N'';
END;
...
BEGIN
    ALTER TABLE [TypeLijsten] ADD [Beschrijving2] nvarchar(max) NOT NULL DEFAULT N'';
END;
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[TypeLijsten]') AND [c].[name] = N'Beschrijving1');
    IF @var27 IS NOT NULL EXEC(N'ALTER TABLE [TypeLijsten] DROP CONSTRAINT [' + @var27 + '];');
    ALTER TABLE [TypeLijsten] DROP COLUMN [Beschrijving1];
END;
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[TypeLijsten]') AND [c].[name] = N'Beschrijving2');
    IF @var28 IS NOT NULL EXEC(N'ALTER TABLE [TypeLijsten] DROP CONSTRAINT [' + @var28 + '];');
    ALTER TABLE [TypeLijsten] DROP COLUMN [Beschrijving2];
END;
...
BEGIN
    EXEC sp_rename N'[Leveranciers].[Code3]', N'Code', 'COLUMN';
END;
